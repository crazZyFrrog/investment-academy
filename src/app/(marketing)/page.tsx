...
export default function HomePage() {
  return (
    <main className="relative overflow-hidden">
      ...
      <section className="relative mx-auto grid min-h-[640px] max-w-7xl items-center gap-12 px-6 pb-20 pt-20 lg:grid-cols-[1.05fr_0.95fr] lg:px-10 lg:pb-28 lg:pt-28">
        <div className="relative z-10">
          <FadeIn>
            <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-[#a8ff16]/25 bg-[#a8ff16]/8 px-3 py-1.5 text-xs font-medium text-[#caff72]">
              <span className="size-1.5 rounded-full bg-[#a8ff16] shadow-[0_0_12px_#a8ff16]" />
              Обучение инвестициям без информационного шума
            </div>
          </FadeIn>
          <SlideUp delay={0.06}>
            <h1 className="max-w-3xl font-display text-5xl leading-[0.98] tracking-[-0.045em] text-white sm:text-7xl lg:text-[5.8rem]">
              Стройте решения.
              <br />
          ...
          <SlideUp delay={0.2}>
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <Button size="lg" className="bg-[#a8ff16] text-[#071007] hover:bg-[#c2ff62]" asChild>
                <Link href="/dashboard" prefetch>
                  Начать путь <ArrowRight className="size-4" />
                </Link>
              </Button>
              <Button size="lg" variant="outline" className="border-white/18 bg-white/5 text-white hover:bg-white/10" asChild>
                <Link href="/courses"><Play className="size-4 fill-current" /> Смотреть курсы</Link>
              </Button>
            </div>
          </SlideUp>
          <div className="mt-12 flex flex-wrap gap-x-8 gap-y-3 text-xs text-white/45">
            <span className="flex items-center gap-2"><ShieldCheck className="size-4 text-[#a8ff16]" /> Без обещаний лёгкой прибыли</span>
            <span className="flex items-center gap-2"><Users className="size-4 text-[#a8ff16]" /> Для любого уровня</span>
          </div>
        </div>
      ...
      <section className="border-y border-white/8 bg-[#0b100e]/80">
        <div className="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-white/8 px-6 lg:grid-cols-4 lg:px-10">
      ...
      <section id="about" className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32">
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            ...
            <h2 className="max-w-md font-display text-4xl leading-tight text-white sm:text-5xl">Всё, что нужно для уверенного старта.</h2>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 lg:px-10 lg:pb-32">
        <div className="rounded-2xl border border-[#a8ff16]/18 bg-[#a8ff16]/[0.06] p-8 sm:p-12 lg:flex lg:items-end lg:justify-between">
          <div><Sparkles className="mb-5 size-6 text-[#a8ff16]" /><h2 className="max-w-2xl font-display text-3xl text-white sm:text-4xl">Ваш следующий шаг — не ещё один прогноз.</h2><p className="mt-4 max-w-xl text-white/55">Начните с основ и соберите собственную систему принятия решений.</p></div>
          <Button size="lg" className="mt-8 bg-[#a8ff16] text-[#071007] hover:bg-[#c2ff62] lg:mt-0" asChild><Link href="/dashboard">Открыть академию <ArrowRight className="size-4" /></Link></Button>
        </div>
      </section>
    </main>
  );
...
function Metric({ value, label }: { value: string; label: string }) {
  return <div className="px-4 py-7 first:pl-0 last:pr-0 sm:px-8"><p className="font-display text-2xl text-white sm:text-3xl">{value}</p><p className="mt-1 text-xs text-white/42">{label}</p></div>;
}
